...
// SOMTHING FAILED TO BE DONE WITH US AND WE MADE ANOTHER SOLUTION
...
const createAndSendToken = (realUser, statusCode, res) => {
  const user = { ...realUser };
...
exports.signup = catchAsync(async (req, res, next) => {
  if (await User.findOne({ email: req.body.email })) {
    return next(
      new AppError('This email is already existed try to login instead', 401)
    );
  }

  /* Its a better way to specify the fields you will take and not taking the hole body */
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // Storing the user data into locals (make it accesable to all pug templates)
      res.locals.user = currentUser;
    } catch (err) {
      return next();
    }
